...
- **`inspector-mcp` skill** — register `InspectorMCPServer` for a consumer Xcode project, then use `query` / `resolve` / `snapshot` / `inspect` / `tap` / `list_properties` / `set_property` on the live UIKit hierarchy. Includes `scripts/find_inspector_package.py` to locate the Inspector Swift package under DerivedData.
