...
final class InspectorAutobuiltPanelTests: XCTestCase {
    ...
    func testButtonConfigurationSectionExposesGlassConfigurationBindings() throws {
        ...
        XCTAssertEqual(configuration.buttonSize, .large)
        XCTAssertEqual(configuration.titleAlignment, .trailing)
    ...
    func testNavigationBarAppearanceSectionUsesBindingsAndMutatesBehavior() throws {
        ...
        let resolvedAppearance = try XCTUnwrap(dataSource.appearance)
        ...
        XCTAssertEqual(bindings[7].descriptor.kind, .separator)
        XCTAssertEqual(bindings[9].descriptor.kind, .group)
        XCTAssertEqual(bindings[10].descriptor.kind, .options)
        XCTAssertEqual(bindings[11].descriptor.kind, .stepper)
        XCTAssertEqual(bindings[15].descriptor.kind, .group)
        XCTAssertEqual(bindings[16].descriptor.kind, .options)
        XCTAssertEqual(bindings[17].descriptor.kind, .stepper)
        XCTAssertEqual(bindings[20].descriptor.kind, .preview)
        ...
        bindings[11].apply(.number(16))
        bindings[12].apply(.color(.blue))
        bindings[17].apply(.number(28))
        bindings[18].apply(.color(.green))
        bindings[20].apply(.size(.init(width: 3, height: 4)))

        XCTAssertEqual(resolvedAppearance.backgroundEffect?.style, UIBlurEffect.Style.allCases[1])
        XCTAssertEqual(resolvedAppearance.backgroundColor, .yellow)
        XCTAssertEqual(resolvedAppearance.backgroundImage?.pngData(), image.pngData())
        XCTAssertEqual(try XCTUnwrap(resolvedAppearance.titleTextAttributes[.font] as? UIFont).pointSize, 16, accuracy: 0.001)
        XCTAssertEqual(resolvedAppearance.titleTextAttributes[.foregroundColor] as? UIColor, .blue)
        XCTAssertEqual(try XCTUnwrap(resolvedAppearance.largeTitleTextAttributes[.font] as? UIFont).pointSize, 28, accuracy: 0.001)
        XCTAssertEqual(resolvedAppearance.largeTitleTextAttributes[.foregroundColor] as? UIColor, .green)
        XCTAssertEqual((resolvedAppearance.largeTitleTextAttributes[.shadow] as? NSShadow)?.shadowOffset, .init(width: 3, height: 4))
    }
