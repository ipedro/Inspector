...
When `Debugging` is off: `Inspector` and `InspectorMacros` are not fetched, not compiled, not linked. `import InspectorInterface` still compiles because the macro declarations are gated with `#if INSPECTOR_DEBUGGING` and are simply absent in release — no type errors, no binary overhead. No Run Script phase needed to strip the framework from release archives.
...
The `#if INSPECTOR_DEBUGGING` guards on the customer's annotations are required because the macro declarations themselves are absent in release builds. This is intentional — `@InspectorPanel` is debug-only code.
...
```swift
// Debug bootstrap — already inside #if INSPECTOR_DEBUGGING
...
4. Generates **two nested types** as new members of the annotated class, both wrapped in `#if INSPECTOR_DEBUGGING`.
...
- The `#if INSPECTOR_DEBUGGING` guard uses `IfConfigDeclSyntax` via SwiftSyntax, which is valid in member macro expansions (confirmed pattern in `swift-spyable`; requires `.poundIfToken()` constructor, not string interpolation).
